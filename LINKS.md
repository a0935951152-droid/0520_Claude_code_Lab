...
| 按鈕 | 動作 | 行為 |
|------|------|------|
| ✉ Contact | `openContact()` 開啟 modal | 填寫姓名/Email/留言 → POST 到 Google Apps Script Web App（寫 Drive Sheet）；未設定 endpoint 時 fallback `mailto:`。設定流程不放 repo，問 Claude 或翻本機 `~/github/0520_Claude_code_Lab/CONTACT_SETUP.md`（已 gitignore） |
